...
# -*- coding: utf-8 -*-
"""
🎓 رادار الخدمات الطلابية أونلاين - النسخة النهائية المعدلة (تغطية كاملة للأخطاء الإملائية)
الوظيفة: رصد أي طلب طالب لخدمة عن بُعد (عروض، واجبات، أعذار طبية، برمجة، خصوصي، تقارير، إكسل، تصميم)
المطور: [اسمك]
التاريخ: 2026
"""
...
# ================== 8. ⭐ أنماط نية طلب التنفيذ - شاملة للأخطاء الإملائية ⭐ ==================
EXECUTION_PATTERNS = [
    # ------------------------------------------------------------
    # 1. أفعال الطلب الأساسية مع تغطية الأخطاء (احتاج/احتجاج/ابي/ابغى/ابقى/ابا/محتاج)
    # ------------------------------------------------------------
    r'(احتاج|احتجاج|محتاج|ابي|ابغى|ابقى|ابا|أبي|أبغى|أحتاج)\s+(حد|شخص|واحد|أحد|احد|من|مين|مني)\s+(يسوي|يسوى|يشوي|يكمل|ينفذ|يعمل|يجهز|يخلص|يرسل|يحل|يصل|يصلح|يكتب|يصمم|يبرمج|يترجم|يعدل|يساعد|يساعدي|يدبر|يضبط|يظبط)',

    # ------------------------------------------------------------
    # 2. صيغ "اللي/الي" مع أخطاء إملائية
    # ------------------------------------------------------------
    r'(اللي|الي|الذي)\s+(يقدر|يعرف|يفهم|فيه|عنده|معاه)\s+(يسوي|يسوى|يشوي|يكمل|ينفذ|يعمل|يجهز|يخلص|يرسل|يحل|يصل|يكتب|يصمم|يبرمج|يترجم|يعدل|يساعد)',

    # ------------------------------------------------------------
    # 3. صيغ "مين/من/مني + فعل"
    # ------------------------------------------------------------
    r'(مين|من|مني)\s+(يسوي|يسوى|يشوي|يكمل|ينفذ|يعمل|يجهز|يخلص|يرسل|يحل|يصل|يصلح|يكتب|يصمم|يبرمج|يترجم|يعدل|يساعد|يعرف|يفهم)\s+(لي|لنا)?',

    # ------------------------------------------------------------
    # 4. صيغ "حد/شخص/واحد + فعل" (بدون كلمة طلب)
    # ------------------------------------------------------------
    r'(حد|شخص|واحد|أحد|احد)\s+(يسوي|يسوى|يشوي|يكمل|ينفذ|يعمل|يجهز|يخلص|يرسل|يحل|يصل|يصلح|يكتب|يصمم|يبرمج|يترجم|يعدل|يساعد|يعرف|يفهم|يضبط)',

    # ------------------------------------------------------------
    # 5. طلبات مباشرة للخدمة بدون ذكر "حد" (تشمل الأخطاء الإملائية)
    # ------------------------------------------------------------
    r'(احتاج|احتجاج|محتاج|ابي|ابغى|ابقى|ابا|عندي|مطلوب)\s+(عرض|بوربوينت|واجب|واجبات|مشروع|تقرير|تقارير|بحث|بحوث|تلخيص|ترجمه|برمجه|برمجة|اكسل|وورد|تصميم|خصوصي|عذر\s+طبي|تقرير\s+طبي|شهادة\s+صحيه|مدرس\s+خصوصي|حل\s+واجب|حل\s+اسايمنت|مشروع\s+تخرج|بروجكت)',

    # ------------------------------------------------------------
    # 6. صيغ "يعرف في/خبير في/فاهم في"
    # ------------------------------------------------------------
    r'(يعرف|يفهم|خبير|فاهم|متخصص)\s+في\s+(البرمجه|البرمجة|التصميم|الاكسل|الوورد|العروض|التقارير)',

    # ------------------------------------------------------------
    # 7. أخطاء إملائية شائعة جداً مع الأفعال
    # ------------------------------------------------------------
    r'(يصل|يصلح)\s+(واجب|تكليف|اسايمنت|مشروع)',  # "يصل واجب" يقصد "يحل واجب"
    r'(يساعدي|تساعدني|يساعدني)\s+في',            # "يساعدي" بدون نون
    r'(تسوي|تشوي|يسوي|يشوي)\s+(لي|لنا)\s+(عرض|بوربوينت|واجب|تقرير|بحث)',

    # ------------------------------------------------------------
    # 8. صيغ النداء مع طلب
    # ------------------------------------------------------------
    r'(يا\s+شباب|ياشباب|يا\s+جماعه|ياجماعه|تكفون|تكفى|يخوان|ياخوان|بنات|يابنات)\s+.*?(احتاج|احتجاج|ابي|ابغى|اللي|حد|مين)',

    # ------------------------------------------------------------
    # 9. طلبات عاجلة/ضرورية
    # ------------------------------------------------------------
    r'(عاجل|ضروري|مستعجل|بسرعه|بسرعة|اليوم)\s+.*?(احتاج|ابي|ابغى|حد|مين)',

    # ------------------------------------------------------------
    # 10. طلبات خصوصي (بأي صيغة)
    # ------------------------------------------------------------
    r'(ابي|ابغى|احتاج|محتاج|ابا)\s+خصوصي\s+(في|لمادة|مادة)',
    r'(مدرس|معلم|خصوصي)\s+(فيزياء|رياضيات|كيمياء|احياء|انجليزي|عربي|برمجه|محاسبه|اقتصاد|احصاء|تمريض|طب)',

    # ------------------------------------------------------------
    # 11. طلبات عذر طبي (بأي صيغة)
    # ------------------------------------------------------------
    r'(عذر|تقرير|شهادة)\s+(طبي|طبيه|مرضي|صحيه)\s+(بسعر|برسوم|بفلوس)?',

    # ------------------------------------------------------------
    # 12. صيغ إضافية: "دور لي"، "ابحث عن"
    # ------------------------------------------------------------
    r'(دور|ابحث|أبحث)\s+(لي|لنا)?\s+(عن|على)\s+(حد|شخص|واحد)\s+(يسوي|يكمل|ينفذ)',
]

# ================== 9. ⭐ القائمة السوداء - تحديث لاستثناء الخدمات المطلوبة ⭐ ==================
BLACKLIST_KEYWORDS = {
    # إعلانات وخدمات تجارية (لا تشمل كلمات الخدمات نفسها مثل "عذر طبي" أو "خصوصي")
...
def analyze_message(text: str) -> tuple[bool, str, str]:
    ...
    # ❌ الفلتر 3: الاستفسارات العامة (إذا بدأت بأداة استفهام وليس معها طلب)
    ...
    # ✅ الفلتر 4: البحث عن نية طلب التنفيذ
