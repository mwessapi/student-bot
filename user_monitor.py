...
# -*- coding: utf-8 -*-
"""
🎓 رادار الخدمات الطلابية أونلاين - النسخة النهائية الشاملة للهجة السعودية
...
# ================== 8. ⭐ أنماط نية طلب التنفيذ - تغطية شاملة للهجة السعودية ⭐ ==================
EXECUTION_PATTERNS = [
    # ------------------------------------------------------------
    # 1. صيغ "أبي/أبغى/احتاج + حد + فعل" (الأساسية)
    # ------------------------------------------------------------
    r'(ابي|ابغى|ابغا|احتاج|محتاج|ابا)\s+(حد|شخص|واحد|أحد|احد)\s+(يسوي|يكمل|ينفذ|يعمل|يجهز|يخلص|يرسل|يحل|يكتب|يصمم|يبرمج|يترجم|يعدل|يساعد|يدبر|يضبط|يظبط)',
    r'(ابي|ابغى|ابغا|احتاج|محتاج|ابا)\s+(حد|شخص|واحد|أحد|احد)\s+(يشوي|يكمّل|ينفّذ)',  # أخطاء إملائية شائعة

    # ------------------------------------------------------------
    # 2. صيغ "اللي + فعل" (شائعة جدًا: "اللي يقدر يسوي لي")
    # ------------------------------------------------------------
    r'(اللي|الي)\s+(يقدر|يعرف|يفهم|فيه|عنده)\s+(يسوي|يكمل|ينفذ|يعمل|يجهز|يخلص|يرسل|يحل|يكتب|يصمم|يبرمج|يترجم|يعدل|يساعد)',

    # ------------------------------------------------------------
    # 3. صيغ "حد/مين + فعل + لي/لنا" (مثل: "حد يسوي لي بحث")
    # ------------------------------------------------------------
    r'(حد|شخص|واحد|أحد|احد|مين|من)\s+(يسوي|يكمل|ينفذ|يعمل|يجهز|يخلص|يرسل|يحل|يكتب|يصمم|يبرمج|يترجم|يعدل|يساعد|يدبر|يضبط)\s*(لي|لنا)?',

    # ------------------------------------------------------------
    # 4. صيغ النداء الجماعي مع طلب (يا شباب، تكفون، يخوان)
    # ------------------------------------------------------------
    r'(يا\s+شباب|ياشباب|يا\s+جماعه|ياجماعه|تكفون|تكفى|يخوان|ياخوان|بنات|يابنات)\s+.*?(ابي|ابغى|احتاج|محتاج|اللي|حد|مين)',

    # ------------------------------------------------------------
    # 5. طلبات مباشرة للخدمة بدون ذكر "حد" (مثل: "ابي عرض بوربوينت")
    # ------------------------------------------------------------
    r'(ابي|ابغى|ابغا|احتاج|محتاج|ابا|عندي|مطلوب)\s+(عرض|بوربوينت|واجب|مشروع|تقرير|بحث|تلخيص|ترجمه|برمجه|اكسل|وورد|خصوصي|عذر\s+طبي|تقرير\s+طبي|شهادة\s+صحيه|مدرس\s+خصوصي|حل\s+واجب|حل\s+اسايمنت|مشروع\s+تخرج|بروجكت)',

    # ------------------------------------------------------------
    # 6. طلبات المساعدة العامة (ساعدوني، اللي يعرف)
    # ------------------------------------------------------------
    r'(ساعدوني|ساعدني|اللي\s+يعرف|اللي\s+فاهم|اللي\s+عنده\s+خبره)\s+.*?(يسوي|يكمل|يحل|يكتب|يصمم)',

    # ------------------------------------------------------------
    # 7. صيغ خاصة للخدمات الشائعة (عذر طبي، خصوصي، مشروع)
    # ------------------------------------------------------------
    r'(ابي|ابغى|احتاج|محتاج)\s+(عذر\s+طبي|تقرير\s+طبي|شهادة\s+صحيه|خصوصي|مدرس|مشروع\s+تخرج|بروجكت|حل\s+واجب)',

    # ------------------------------------------------------------
    # 8. تغطية الأخطاء الإملائية الشائعة (يشوي، تسوي بدل يسوي)
    # ------------------------------------------------------------
    r'(حد|شخص|واحد|اللي)\s+(يشوي|تشوي|يسوي|تسوي)\s+(لي|لنا)?',
    r'(ابي|ابغى|احتاج)\s+(حد|شخص)\s+(يشوي|تشوي)',

    # ------------------------------------------------------------
    # 9. طلبات تحتوي على "عاجل" أو "ضروري" (تعزيز الثقة)
    # ------------------------------------------------------------
    r'(عاجل|ضروري|مستعجل|بسرعه|بسرعه)\s+.*?(ابي|ابغى|احتاج|حد|اللي)',

    # ------------------------------------------------------------
    # 10. صيغ البحث عن منفذ (أبحث عن، دور لي)
    # ------------------------------------------------------------
    r'(ابحث|أبحث|دور|لقيت)\s+(لي|لنا)?\s+(عن|على)\s+(حد|شخص|واحد)\s+(يسوي|يكمل|ينفذ)',
]
